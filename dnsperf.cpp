...
main(int argc, char *argv[])
{
	...
	if (conn.connect("mysql_cpp_data", "localhost",
		...
		mysqlpp::Query query_domains = conn.query("select * from domains");
		...
		if (res_domains) {
			...
			cerr << "Failed to get dnsqueries table: " << query_domains.error() << endl;
			return 1;
		}

		for (i=0; i<res_domains.num_rows(); i++) {

			timevalue = resolve(res_domains[i]["domain"].c_str(), argv[1]);
			//gettimeofday(&currenttime,NULL);

			try {
				mysqlpp::Query query = conn.query();
				cout << "Updating mysql table dnsqueries..." << endl;
				...
				cout << "Populating dnsqueries table..." << flush;
				char date[20];
				time_t tm;
				struct tm *tmp;
				tm = time(NULL);
				...
				query.execute(res_domains[i]["domain"].c_str(), timevalue, date);

				cout << "inserted " << conn.count_rows("dnsqueries") << " rows." << endl;
			}
		...
		mysqlpp::Query query_queries= conn.query("select * from dnsqueries");
		...
		if (res_queries) {
			...
			for (size_t i = 0; i < res_queries.num_rows(); ++i) {
				cout << setw(30) << res_queries[i]["domain"] << ' ' <<
						...
						setw(9) << res_queries[i]["timestamp"] <<
						endl;
			}
		}
		else {
			cerr << "Failed to get dnsqueries table: " << query_queries.error() << endl;
			return 1;
		}
...
#if 1
unsigned long
resolve(const char *domainname, const char *domaintoquery)
{
	...
	ldns_rdf_deep_free(domain);
		...
		if (!query_results) {
	        ...
	        for (i = 0; i < ldns_rr_list_rr_count(query_results); i++) {
                	ns_name = ldns_rr_rdf(ldns_rr_list_rr(query_results, i), 0);
			printf("id = %d\n", i);
			if (!ns_name) { printf("error, ns_name is null\n"); }
                        iplist = ldns_get_rr_list_addr_by_name(res, ns_name, LDNS_RR_CLASS_IN, 0);
                        (void) ldns_resolver_push_nameserver_rr_list(actual_res, iplist);
...
#endif
        	s1=t2.tv_sec * 1000.0 +(t2.tv_usec/1000.0) - (t1.tv_sec * 1000.0 + (t1.tv_usec/1000.0));
...
//		query.execute(1, "nanos.gr");
		...
		cout << "inserted " << con.count_rows("dnsqueries") << " rows." << endl;
		cout << "inserted " << con.count_rows("domains") << " rows." << endl;
